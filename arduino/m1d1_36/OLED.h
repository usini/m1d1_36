...
/*!
 *  @brief draw what buttons was pressed
 * 
 *  @param button_id
 *         Buttons id from (0 to 10)
 *  @param state
 *         Was the button toggled ?
*/
void drawButtons(int button_id, bool state) {
  int x = button_id * 10;

  //As buttons 6/7/8/9 are used for transpose/channel do not draw
  if (button_id >= 6) {
    x = x + 28;
  }

  if (state) {
    display.fillRect(x, 0, 10, 16, SSD1306_WHITE); // Draw filled box
  } else {
    display.fillRect(x, 0, 10, 16, SSD1306_BLACK); // Erase filled box
  }
  screen_update = true; // Tell screen to update at end of loop
}

/*!
 *  @brief display potentiometers values on screen
*/
void drawPots() {
  display.fillRect(85, 0, 20, 8, SSD1306_BLACK);
  display.setCursor(85, 0);
  String string = String(pots_val[0]);
  display.write(string.c_str());
  display.fillRect(85, 8, 30, 8, SSD1306_BLACK);
  display.setCursor(85, 8);
  string = String(pots_val[1]);
  display.write(string.c_str());

  display.fillRect(105, 0, 20, 8, SSD1306_BLACK);
  display.setCursor(105, 0);
  string = String(pots_val[2]);
  display.write(string.c_str());
  display.fillRect(105, 8, 20, 8, SSD1306_BLACK);
  display.setCursor(105, 8);
  string = String(pots_val[3]);
  display.write(string.c_str());
